macro_rules! transpose_case {
    ($name:ident, $r:literal, $c:literal) => {
        ...
        fn $name() {
            let source: [[f32; $c]; $r] =
                core::array::from_fn(|i| core::array::from_fn(|j| (1 + i * $c + j) as f32));
            let actual: [[f32; $r]; $c] = matrix_to_rows!(matrix_from_rows!(source).transpose());
            let expected = core::array::from_fn(|j| core::array::from_fn(|i| source[i][j]));
...
fn reference_inverse<const D: usize>(source: [[f32; D]; D]) -> [[f32; D]; D] {
    let mut a = source;
    let mut inverse = core::array::from_fn(|i| core::array::from_fn(|j| (i == j) as u8 as f32));
    for pivot in 0..D {
        let best = (pivot..D)
...
fn assert_matrix_close<const D: usize>(actual: [[f32; D]; D], expected: [[f32; D]; D]) {
    for i in 0..D {
        for j in 0..D {
            let tolerance = 2.0e-4 * expected[i][j].abs().max(1.0);
            assert!((actual[i][j] - expected[i][j]).abs() <= tolerance,
                "({i}, {j}): actual={}, expected={}", actual[i][j], expected[i][j]);
        }
    }
}
...
fn diagonal_preserves_logical_coordinates() {
    ...
    assert_eq!(<[f32; 2]>::from(m2.diagonal()), [2.0, 7.0]);
    assert_eq!(<[f32; 3]>::from(m3.diagonal()), [2.0, 11.0, 23.0]);
    assert_eq!(<[f32; 4]>::from(m4.diagonal()), [2.0, 13.0, 31.0, 53.0]);
}
...
fn all_square_inverses_and_determinants_match_nalgebra() {
    macro_rules! compare { ($d:literal, $source:expr) => {{
        let source: [[f32; $d]; $d] = $source;
        let flat: Vec<f32> = source.iter().flatten().copied().collect();
        let reference = nalgebra::SMatrix::<f32, $d, $d>::from_row_slice(&flat);
        let matrix: Matrix<f32, $d, $d> = matrix_from_rows!(source);
